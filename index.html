<!doctype html>
<html lang="en-US">
<head>
<meta charset="UTF-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>IaC</title>
<link href="css/singlePageTemplate.css" rel="stylesheet" type="text/css">
<!--The following script tag downloads a font from the Adobe Edge Web Fonts server for use within the web page. We recommend that you do not modify it.-->
<script>var __adobewebfontsappname__="dreamweaver"</script>
<script src="http://use.edgefonts.net/source-sans-pro:n2:default.js" type="text/javascript"></script>
<!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
<!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
<!--[if lt IE 9]>
      <script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
      <script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
    <![endif]-->

<!-- Terence Scripting -->
<script type="text/javascript">

        // Wait 
        window.onload = function() {

          
          	var micro = document.getElementById("microVM");
			var small = document.getElementById("smallVM");
			var medium = document.getElementById("mediumVM");
			var large = document.getElementById("largVM");
          //execute code to load vm in aws with terraform
          micro.onclick = function() 
		  {

            
			  
			  
			  window.location.pathname = "CurrentVMs.php";
			  
			  return false;
			  
          }
		  
		  
		  
        }
		
    </script>
	
	
</head>
<body>
<!-- Main Container -->
<div class="container"> 
  <!-- Navigation -->
  <header> <a href="index.php">
    <h4 class="logo">WildWest</h4>
  </a>
    <nav>
      <ul>
        <li><a href="index.php">Create</a><a href="#hero"> VMs</a></li>
        <li><a href="CurrentVMs.php">Current</a><a href="#about"> VMs</a></li>
      </ul>
    </nav>
  </header>
  <!-- Hero Section -->
  <section class="hero" id="hero">
    <h2 class="hero_header">InfraStructure_as: <span class="light">Code</span></h2>
    
  </section>
  <!-- About Section -->
  <section class="about" id="about">
    <h2 class="hidden">About</h2>
    <p class="text_column">This is just a quick inteface to illsutrate spinning up a few VMs with different tools. </p>
	 <p class="text_column"><br>
		There is a stict limit of 3 VMs per type to avoid excessive cost. However, current VMs are displayed on the <i>Current VM</i> page and may be destroyed from there. 
	</p>
    <p class="text_column">&nbsp;</p>
  </section>
  <!-- Stats Gallery Section -->
  <div class="teraForm">lets try teraform</div>

	
  <div class="gallery">
	 

    
	<a id="microVM" href="index.php"> 
    <div class="thumbnail">

      <h1 class="stats">Micro</h1>
      <h4>Linux AMI + Apache</h4>
      <p>One line description</p>
	
    </div>
	</a>  
		
	<div class="thumbnail">
      <h1 class="stats">Small</h1>
      <h4>Linux AMI + Apache</h4>
      <p>One line description</p>
    </div>
    <div class="thumbnail">
      <h1 class="stats">Medium</h1>
      <h4>Linux AMI + Apache</h4>
      <p>One line description</p>
    </div>
    <div class="thumbnail">
      <h1 class="stats">Large</h1>
      <h4>Linux AMI + Apache</h4>
      <p>One line description</p>
    </div>
  </div>
  <!-- Parallax Section -->
  <!-- More Info Section -->
  <footer>
    <article class="footer_column"> </article>
    <article class="footer_column"> </article>
  </footer>
  <!-- Footer Section -->
  <!-- Copyrights Section -->
  <div class="copyright">&copy;2018 - <strong>IaC</strong></div>
</div>
<!-- Main Container Ends -->
</body>
</html>
